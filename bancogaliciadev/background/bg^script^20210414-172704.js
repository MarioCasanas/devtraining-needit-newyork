...
try {
    ...
    grpardidasPendientes.groupBy('u_partida_presupuestaria'); //Group aggregate by the 'os' field
    grpardidasPendientes.addAggregate('count'); //Count aggregate (only necessary for a count of items of each OS)
    grpardidasPendientes.addQuery('u_estado=NULL^ORu_estado=""');
    
    grpardidasPendientes.query();

    //Se va a iterar por cada Partida Presupuestaria
    while(grpardidasPendientes.next()){

        //Se obtienen todos los Breakdowns de la Partida que estoy parado.
        var grBreakdown = new GlideRecord('u_dxc_temp_breakdown');
        grBreakdown.addQuery('u_estado=NULL^u_partida_presupuestaria='+grpardidasPendientes.u_partida_presupuestaria);
        grBreakdown.query();
        ...
        while (grBreakdown.next()) {
            ...
            //Sumarizo los Meses
            arrayMeses[0]   = arrayMeses[0]  + operacion(operacionPresupuestaria, parseFloat(formatearMonto(grBreakdown.u_enero)));
            arrayMeses[1]   = arrayMeses[1]  + operacion(operacionPresupuestaria, parseFloat(formatearMonto(grBreakdown.u_febrero)));
            arrayMeses[2]   = arrayMeses[2]  + operacion(operacionPresupuestaria, parseFloat(formatearMonto(grBreakdown.u_marzo)));
            arrayMeses[3]   = arrayMeses[3]  + operacion(operacionPresupuestaria, parseFloat(formatearMonto(grBreakdown.u_abril)));
            arrayMeses[4]   = arrayMeses[4]  + operacion(operacionPresupuestaria, parseFloat(formatearMonto(grBreakdown.u_mayo)));
            arrayMeses[5]   = arrayMeses[5]  + operacion(operacionPresupuestaria, parseFloat(formatearMonto(grBreakdown.u_junio)));
            arrayMeses[6]   = arrayMeses[6]  + operacion(operacionPresupuestaria, parseFloat(formatearMonto(grBreakdown.u_julio)));
            arrayMeses[7]   = arrayMeses[7]  + operacion(operacionPresupuestaria, parseFloat(formatearMonto(grBreakdown.u_agosto)));
            arrayMeses[8]   = arrayMeses[8]  + operacion(operacionPresupuestaria, parseFloat(formatearMonto(grBreakdown.u_septiembre)));
            arrayMeses[9]   = arrayMeses[9]  + operacion(operacionPresupuestaria, parseFloat(formatearMonto(grBreakdown.u_octubre)));
            arrayMeses[10]  = arrayMeses[10] + operacion(operacionPresupuestaria, parseFloat(formatearMonto(grBreakdown.u_noviembre)));
            arrayMeses[11]  = arrayMeses[11] + operacion(operacionPresupuestaria, parseFloat(formatearMonto(grBreakdown.u_diciembre)));
            arrayMeses[12]  = arrayMeses[12] + operacion(operacionPresupuestaria, parseFloat(formatearMonto(grBreakdown.u_total)));

        }

        var j=0;                    
        gs.print('---------------------------------- '+grpardidasPendientes.u_partida_presupuestaria+' ---------------------------------- ');
        while (j<=12) {
        ...
        try {
            ...
            graboBreakdownCalculado(grpardidasPendientes.u_partida_presupuestaria.toString(), grBreakdown.u_ejercicio, arrayMeses);    
        } catch (error) {
            message = error.message;
